...
#def proxify(useproxy):
#	if useproxy==False:	
#		proxydata=getproxy()
#		proxydata=cleanrata(proxydata)
#		return proxydata
...
def using_requests(request_url, apisign):
       ...
       # if useproxy==True:
        #    global proxydata
         #   huexy=random.choice(proxydata)
          #  global proxy		
           # proxy={'http':'http://'+huexy['ip']+':'+huexy['port']}
        #proxy={'http':'http://217.197.251.191:8080'}
        
          #  printing=False
         #   if printing==True:
          #      global apiused 
          #      print('api used '+apiused+'proxy used N'+str(m)+' '+str(proxy)+' for ' +str(request_url))
                #print(proxy)
				#print()
           # hore=requests.get(request_url,headers={"apisign": apisign},proxies=proxy)
            
      #  else:
        hore=requests.get(request_url,headers={"apisign": apisign})
...
class Bittrex(object):
    ...
    def get_orderbook(self, market, depth_type, depth =10):
        """
...
def cleanrata(proxydata):
	global clean
	clean=[]
	#import bittrex
	#getbalance, measure time, exclude
	print('CLEANING')
	
	for x in range(0, len(proxydata)-1):
		time1=time.clock()
		proxy=proxydata[x]
			
		api =  Bittrex(api_key1o,api_secret1o)	
		data = api.get_balances()
		time2=time.clock()
		timd=time2-time1
		global proxycalllimit
		if timd<proxycalllimit:
			
			print('adding proxy. timd '+str(timd))
			clean.append(proxy)
